...
function checkNameLength(name, maxLength) {
    if (name.length > maxLength) return `Максимальная длина имени ${maxLength} символов!`;
    if (name.length <= 0) return "Необходимо заполнить поле с именем!";
    return false;
}
function checkDateFormat(date) {
    if (/^(0[1-9]|[12][0-9]|3[01])[- /.](0[1-9]|1[012])[- /.](19|20)\d\d$/.test(date) != true && date.length > 0) return "Дата должна быть записана в формате ДД.ММ.ГГГГ";
    return false;
}
function checkCommentLength(comment) {
    if (comment.textLength == 0) return "Поле с комментарием должно быть заполнено!";
    return false;
}
function getDate(date) {
    let d = new Date(), h = (d.getHours() < 10 ? "0" : "") + d.getHours(), m = (d.getMinutes() < 10 ? "0" : "") + d.getMinutes(), cDate = "", resString = "";
    if (date) cDate = new Date(`${date[3]}${date[4]}/${date[0]}${date[1]}/${date.slice(6)}`);
    else return `сегодня, ${h}:${m}`;
    let timeStamp = d.getTime() - cDate.getTime();
    if (timeStamp < 86400000 && timeStamp >= 0) resString = `сегодня, ${h}:${m}`;
    else if (timeStamp < 172800000 && timeStamp >= 0) resString = `вчера, ${h}:${m}`;
